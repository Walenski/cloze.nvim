...
local function send_note_to_anki(note)
	...
	local is_duplicate = decoded.error == "cannot create note because it is a duplicate"
	if is_duplicate then
		print("⚠️ Duplicate note skipped: " .. (note.fields.Front or note.fields.Text or "[no content]"))
	elseif decoded.error and decoded.error ~= vim.NIL then
		print("❌ Error: " .. tostring(decoded.error))
	else
		print("✅ Added: " .. (note.fields.Front or note.fields.Text or "[no content]"))
	end
end
